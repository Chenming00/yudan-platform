...
当前已完成平台工程、A02 数据库底座、A03 认证权限、A04 统一账本和 A05 Cloudflare R2 资源层：包括邀请码注册、William 超级管理员、服务端 RBAC、收入/支出/退款与跨模块拆分，以及私有资源直传、确认校验、授权下载、业务关联和可重试删除。`YUDAN` 到保留的 `yudan-wupin` 数据库使用幂等迁移脚本，生产迁移必须在备份和演练后执行。
