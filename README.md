...
当前已完成平台工程、A02 数据库底座、A03 认证权限、A04 统一账本、A05 Cloudflare R2 资源层和 A06 儿童保健模块：包括邀请码注册、William 超级管理员、服务端 RBAC、统一支出与跨模块拆分、私有资源生命周期，以及宝宝档案、生长、疫苗和健康记录 API。`YUDAN` 到保留的 `yudan-wupin` 数据库使用幂等迁移脚本，生产迁移必须在备份和演练后执行。
