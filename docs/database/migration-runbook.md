...
5. 临时配置 `LEGACY_YUDAN_DATABASE_URL`、`DIRECT_URL`、`TARGET_HOUSEHOLD_ID`、`TARGET_ACTOR_USER_ID` 和 `TARGET_BABY_NAME`，真实值不得写入仓库。
6. 执行 `npm run migration:yudan:dry-run` 并核对来源报告。
7. 在维护窗口执行 `npm run migration:yudan:apply`。脚本使用单个目标事务、事务级咨询锁和 5 秒锁等待；任一步失败都会整体回滚。
8. 核对数量、总金额、逐月金额、成长记录和接种记录。
9. 旧站改为只读，观察一周后再决定是否下线；不删除旧数据库。
